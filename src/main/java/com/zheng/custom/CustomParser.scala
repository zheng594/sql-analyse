...
class CustomParser(val delegate: ParserInterface) extends ParserInterface {
     ...
     */
    // scalastyle:on
    protected def parse[T](command: String)(toResult: CustomSqlBaseParser => T): T = {
        val lexer = new CustomSqlBaseLexer(new UpperCaseCharStream(CharStreams.fromString(command)))
        lexer.removeErrorListeners()
        lexer.addErrorListener(ParseErrorListener)

        val tokenStream = new CommonTokenStream(lexer)
        val parser = new CustomSqlBaseParser(tokenStream)
